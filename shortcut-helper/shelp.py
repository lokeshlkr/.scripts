...
def zenity(command):
    ...
    res = str(stdout)[
        2:-3
    ].strip()  # slicing to get rid of quotes and new line character
    return res


def notify(text, fg="", bg="", style="", end="\n", level=Level.TERMINAL):
    if level in (Level.TERMINAL, Level.BOTH):
    ...
    if level in (Level.NOTIFICATION, Level.BOTH):
        ...
        run(
            f"notify-send ShortcutHelper \"<span font-family='Stranger Nerd Font Mono'>{text}</span>\" -i keyboard {timeout}"
        )
...
def rustnew():
    ...
    if not openineditor(paths["rust"]):
        notify(
            "Something went wrong, new practice project could not be created.",
            fg="red",
            style="bold",
            level=Level.BOTH,
        )
...
def restart(name=None, path=None):
    ...
    if path is None:
        path = name
    if len(rest.strip()) == 0 and name is None and path is None:
